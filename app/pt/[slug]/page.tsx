...
export async function generateMetadata({ params }: SlugPageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE.baseUrl}/pt/${slug}`
  ...
  if (resolved && 'sound' in resolved) {
    ...
    const title = `${sound.name} Efeito Sonoro | Botões de Som Instantâneos`
    const description = `Baixe e reproduza o botão de som ${sound.name}. Efeito sonoro perfeito para memes, games, streaming e compartilhamento de conteúdo viral em todas as plataformas.`
    return {
      title,
      description,
      metadataBase: new URL(SITE.baseUrl),
      ...
      openGraph: {
        ...
        description,
        url: canonicalUrl,
        images: [{ url: "/sound.jpg", width: 1200, height: 630, alt: `${SITE.name} - Botão de som` }],
      },
...
export default async function PortugueseSlugPage({ params, searchParams }: SlugPageProps) {
  const { slug } = await params
  ...
  if (slug.endsWith('-soundboard')) {
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": category.name, "item": `${BASE}/pt/${slug}` },
      ],
    }
      ...
      "itemListElement": sounds.slice(0, 10).map((s, idx) => {
        ...
        return {
          ...
          "item": {
            "@type": "AudioObject",
            "name": s.name,
            "description": `Botão do soundboard ${category.name}`,
            "url": `${BASE}/${soundSlug}`,
          },
        }
      }),
