...
export async function generateMetadata({ params }: SlugPageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE.baseUrl}/fr/${slug}`
  ...
  if (resolved && 'sound' in resolved) {
    ...
    const title = `${sound.name} Effet Sonore | Boutons Sonores Instantanés`
    const description = `Téléchargez et jouez le bouton sonore ${sound.name}. Effet sonore parfait pour mèmes, gaming, streaming et partage de contenu viral sur toutes les plateformes.`
    return {
      title,
      description,
      metadataBase: new URL(SITE.baseUrl),
      ...
      openGraph: {
        ...
        description,
        url: canonicalUrl,
        images: [{ url: "/sound.jpg", width: 1200, height: 630, alt: `${SITE.name} - Bouton sonore` }],
      },
...
export default async function FrenchSlugPage({ params, searchParams }: SlugPageProps) {
  const { slug } = await params
  ...
  if (slug.endsWith('-soundboard')) {
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": category.name, "item": `${BASE}/fr/${slug}` },
      ],
    }
      ...
      "itemListElement": sounds.slice(0, 10).map((s, idx) => {
        ...
        return {
          ...
          "item": {
            "@type": "AudioObject",
            "name": s.name,
            "description": `Bouton du soundboard ${category.name}`,
            "url": `${BASE}/${soundSlug}`,
          },
        }
      }),
