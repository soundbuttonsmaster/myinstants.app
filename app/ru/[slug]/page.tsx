...
export async function generateMetadata({ params }: SlugPageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE.baseUrl}/ru/${slug}`
  ...
  if (resolved && 'sound' in resolved) {
    ...
    const title = `${sound.name} Звуковой Эффект | Мгновенные Звуковые Кнопки`
    const description = `Скачайте и воспроизведите кнопку ${sound.name}. Идеальный звуковой эффект для мемов, гейминга, стриминга и распространения вирусного контента на всех платформах.`
    return {
      title,
      description,
      metadataBase: new URL(SITE.baseUrl),
      ...
      openGraph: {
        ...
        description,
        url: canonicalUrl,
        images: [{ url: "/sound.jpg", width: 1200, height: 630, alt: `${SITE.name} - Звуковая кнопка` }],
      },
...
export default async function RussianSlugPage({ params, searchParams }: SlugPageProps) {
  const { slug } = await params
  ...
  if (slug.endsWith('-soundboard')) {
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": category.name, "item": `${BASE}/ru/${slug}` },
      ],
    }
      ...
      "itemListElement": sounds.slice(0, 10).map((s, idx) => {
        ...
        return {
          ...
          "item": {
            "@type": "AudioObject",
            "name": s.name,
            "description": `Кнопка саундборда ${category.name}`,
            "url": `${BASE}/${soundSlug}`,
          },
        }
      }),
