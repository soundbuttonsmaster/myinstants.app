...
export default function FavoritesPageClient() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    apiClient.getUserFavorites(token, 1, pageSize).then((res) => {
      if (cancelled) return
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await apiClient.getUserFavorites(token, nextPage, pageSize)
      const data = res.data as { results?: Sound[]; next?: string | null }
  ...
  return (
    <>
      ...
      <p className="mb-6 text-slate-600 dark:text-slate-300">
        Sounds you hearted show up here. Add more from the home page or search!
      </p>
      {loading && sounds.length === 0 ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 p-8 sm:p-12 text-center">
          <p className="text-slate-600 dark:text-slate-300 mb-4">
            You haven&apos;t added any favorites yet. Click the heart on any sound to add it here!
          </p>
          <Link
            href="/"
            className="inline-block rounded-xl bg-gradient-to-r from-pink-500 to-rose-600 px-6 py-3 font-semibold text-white hover:from-pink-600 hover:to-rose-700 transition-all"
          >
            Explore sounds
          </Link>
        </div>
          ...
          {hasMore && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                onClick={loadMore}
                disabled={loading}
                className="rounded-xl bg-gradient-to-r from-pink-500 to-rose-600 px-8 py-3 font-semibold text-white hover:from-pink-600 hover:to-rose-700 disabled:opacity-70 transition-all"
              >
