...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  try {
    ...
    if (response?.data) {
      ...
      const canonicalUrl = `${SITE.baseUrl}/es/category/${id}`
...
export default async function SpanishCategoryPage({ params, searchParams }: CategoryPageProps) {
  const { id } = await params
  ...
  try {
    ...
    if (categoryResponse?.data) {
      categoryName = (categoryResponse.data as { name: string }).name || "Categoría"
    }
  } catch (error) {
  ...
  const categoryUrl = `${BASE}/es/category/${categoryId}`
    ...
    "itemListElement": sounds.slice(0, 10).map((s, idx) => {
      ...
      return {
        ...
        "item": {
          "@type": "AudioObject",
          "name": s.name,
          "description": `Botón del soundboard ${categoryName}`,
          "url": `${BASE}/${slug}`,
        },
      }
    }),
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-6">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">
            {count > 0
              ? `${count.toLocaleString('es')} sonido${count !== 1 ? "s" : ""} en esta categoría`
              : `Explora los mejores botones del soundboard ${categoryName}. Gratis para reproducir y descargar.`}
          </p>
        </div>
        ...
        {sounds.length > 0 ? (
          <>
            ...
            <Pagination
              ...
              baseUrl={`/es/category/${categoryId}`}
            />
          </>
        ) : (
          <div className="py-16 text-center">
            <p className="text-slate-500 dark:text-slate-400">No hay sonidos en esta categoría.</p>
          </div>
        )}
      </main>
